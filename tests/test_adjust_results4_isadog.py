...
# Path to dognames.txt relative to project root
dogfile = get_project_path("src/intropyproject_classify_pet_images/data/dognames.txt")
...
def test_adjust_results4_isadog(results, adjusted_results4_isadog):
    ...
    for k in results:
        assert results[k] == adjusted_results4_isadog[k], (
            f"Mismatch for {k}: {results[k]} != {adjusted_results4_isadog[k]}"
        )
